...
ggpairs(wine, upper=list(params= c(size=3)), lower=list(params=c(shape = I('.'), outlier.shape = I('.'))))+ theme(axis.ticks = element_blank(),axis.text.x = element_blank(),axis.text.y = element_blank(), panel.grid.major = element_blank())
```

####Unfortunately I was not able to find a good way to adjust the labels of the variables to make them more readable.
...
ggplot(aes(x = pH, y = residual.sugar), data= wine)+geom_point()
```
...
ggplot(aes(factor(quality),volatile.acidity), data= wine)+
  geom_boxplot()+
  geom_point(aes(size=alcohol, color=alcohol))+
